...
import { Tile, ScreenPosition, IsoCoordinates, TileTypes } from '../utils/types';
...
const EnhancedIsometricMap: React.FC = () => {
  ...
  const isoToScreen = (x: number, y: number, z: number = 0): ScreenPosition => {
    const screenX = (x - y) * mapData.tileSize / 2;
    const screenY = (x + y) * mapData.tileSize / 4 - z * mapData.tileSize / 4;
    return { 
      x: screenX + 400, // Center the map
      y: screenY + 200
    };
  };

  // Convert screen coordinates to isometric coordinates
  const screenToIso = (screenX: number, screenY: number): IsoCoordinates => {
    const relativeX = screenX - 400;
    const relativeY = screenY - 200;
    const x = (relativeX / (mapData.tileSize / 2) + relativeY / (mapData.tileSize / 4)) / 2;
    const y = (relativeY / (mapData.tileSize / 4) - relativeX / (mapData.tileSize / 2)) / 2;
  ...
  const drawTile = (ctx: CanvasRenderingContext2D, tile: Tile, isSelected: boolean = false): void => {
    const screenPos = isoToScreen(tile.x, tile.y, tile.z);
