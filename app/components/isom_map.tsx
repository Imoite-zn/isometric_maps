...
import { Tile, MapData, ScreenPosition, IsoCoordinates } from '../utils/types';
...
const IsometricMap: React.FC = () => {
  ...
  const isoToScreen = (x: number, y: number, z: number = 0): ScreenPosition => {
    ...
    const screenY = (x + y) * tileSize / 4 - z * tileSize / 2;
  ...
  useEffect(() => {
    ...
    if (selectedTile) {
      const screenPos = isoToScreen(selectedTile.x, selectedTile.y, selectedTile.z);
